...
def parse_command_file(filepath: str) -> str:
    commands = collect_commands(filepath)
    function_docs = ""
...
def get_tools(filepath_list: list) -> list:
    tools = []
    tools.extend([collect_commands(filepath) for filepath in filepath_list if filepath is not None])
...
DEFAULT_TOOLS = get_tools(DEFAULT_SCRIPT_FILEPATHS)
